...
class _SpeechToTextPageState extends State<SpeechToTextPage> with GeneralLibFlutterStatefulWidget {
  ...
  Future<void> initialized() async {
    ...
    await Future(() async {
      final bool isLoadedModel = generalAiSpeechToText.loadWhisperModel(
        whisperModelPath: () {
          if (Dart.isLinux) {
            return "/home/galaxeus/Documents/big-data/ai/whisper-ggml/ggml-small.bin";
          }
          return File(path.join(generalFrameworkClientFlutterAppDirectory.app_support_directory.path, "ggml-small.bin")).path;
        }(),
      );
  ...
  final File fileAudioRecord = () {
    return File(path.join(generalFrameworkClientFlutterAppDirectory.app_support_directory.path, "record.wav"));
  }();
                      ...
                      menuBuilder: (context) {
                        return [
                          MenuContainerGeneralFrameworkWidget.lisTile(
                            ...
                            title: "Support",
                            trailing: Icon(
                              (generalAiSpeechToText.isDeviceSupport() == true) ? Icons.verified : Icons.close,
                            ),
                          ),
                          MenuContainerGeneralFrameworkWidget.lisTile(
                            context: context,
                            ...
                            title: "Record Audio",
                            trailing: Icon(
                              (generalFlutter.system_audio.isSupport() == true) ? Icons.verified : Icons.close,
                            ),
                          ),
                        ];
                      },
                              ...
                              handleFunction(
                                onFunction: (context, statefulWidget) async {
                                  final File fileWav = File(
                                    () {
                                      if (Dart.isLinux) {
                                        return "/home/galaxeus/Documents/galaxeus/app/whisper_library/native_lib/lib/whisper.cpp/samples/jfk.wav";
                                      }
                                      return path.join(generalFrameworkClientFlutterAppDirectory.app_support_directory.path, "jfk.wav");
                                    }(),
                                  );
                                  ...
                                  try {
                                    transcribeFromExampleJFKToJson = await generalAiSpeechToText.transcribeToJson(
                            ...
                            onTap: () {
                              handleFunction(
                                onFunction: (context, statefulWidget) async {
                                  ...
                                  DateTime dateTime = DateTime.now();
                                  try {
                                    transcribeFromRecordToJson = await generalAiSpeechToText.transcribeToJson(
                                      fileWav: fileWav.path,
                                      useCountProccecors: 1,
                                      useCountThread: 4,
                                    );
  ...
  GeneralAudioRecorder getGeneralAudioRecorder() {
    ...
    generalAudioRecorder = generalFlutter.system_audio.generalAudio.createRecordOrGetRecord(
