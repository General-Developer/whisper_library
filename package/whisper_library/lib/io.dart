...
class WhisperLibrary extends WhisperLibraryBase {
  ...
  ///
  static late final WhisperLibrarySharedBindingsByGeneralDeveloper _whisperLibrarySharedBindingsByGeneralDeveloper;
  ...
  Future<void> ensureInitialized() async {
    ...
    try {
      _whisperLibrarySharedBindingsByGeneralDeveloper = WhisperLibrarySharedBindingsByGeneralDeveloper(
        DynamicLibrary.open(
          libraryWhisperPath,
        ),
      );
  ...
  bool loadWhisperModel({
    ...
    final whisperModelPathNative = WhisperLibrary._whisperModelPath.toNativeUtf8().cast<Char>();
    {
      ...
      if (whisperModelContext != null) {
        /// release memory
        _whisperLibrarySharedBindingsByGeneralDeveloper.whisper_free(whisperModelContext);
      }
    }

    final cparams = _whisperLibrarySharedBindingsByGeneralDeveloper.whisper_context_default_params();
    cparams.use_gpu = WhisperLibrary._isUseGpu;
    cparams.gpu_device = WhisperLibrary._gpuDevice;

    final whisperModelContext = _whisperLibrarySharedBindingsByGeneralDeveloper.whisper_init_from_file_with_params(whisperModelPathNative, cparams);
    ...
    _whisperLibrarySharedBindingsByGeneralDeveloper.whisper_ctx_init_openvino_encoder(whisperModelContext, nullptr, openVinoEncoderDeviceNative, nullptr);
    return true;
  }
  ...
  Future<Map> transcribeToJson({
    ...
    final List<double> filePcm32 = utilsWavToNumpyArraywavToNp(fileWav);
    if (filePcm32.isEmpty) {
      return {
        "@type": "error",
      };
    }
    final pointerFilePcm32 = _utilsListToPointer(filePcm32);
    if (_whisperLibrarySharedBindingsByGeneralDeveloper.whisper_is_multilingual(whisperModelContext) == 0) {
      language = "en";
      isTranslate = false;
    }
    int nThreads = () {
    ...
    int maxContext = -1;
    final wparams = _whisperLibrarySharedBindingsByGeneralDeveloper.whisper_full_default_params(whisper_sampling_strategy.WHISPER_SAMPLING_GREEDY);
    ...
    final int resultInference = () {
      if (nProccecors > 1) {
        return WhisperLibrary._whisperLibrarySharedBindingsByGeneralDeveloper.whisper_full_parallel(whisperModelContext, wparams, pointerFilePcm32, filePcm32.length, nProccecors);
      }
      return WhisperLibrary._whisperLibrarySharedBindingsByGeneralDeveloper.whisper_full(
        whisperModelContext,
        wparams,
        pointerFilePcm32,
        filePcm32.length,
    ...
    final int nSegments = WhisperLibrary._whisperLibrarySharedBindingsByGeneralDeveloper.whisper_full_n_segments(whisperModelContext);
    String result = "";
    for (int i = 0; i < nSegments; ++i) {
      final text = WhisperLibrary._whisperLibrarySharedBindingsByGeneralDeveloper.whisper_full_get_segment_text(whisperModelContext, i);
    ...
    final Map resultJson = {
      ...
      "language": WhisperLibrary._whisperLibrarySharedBindingsByGeneralDeveloper.whisper_lang_str_full(WhisperLibrary._whisperLibrarySharedBindingsByGeneralDeveloper.whisper_full_lang_id(whisperModelContext)).cast<Utf8>().toDartString(),
  ...
  Pointer<Float> _utilsListToPointer(List<double> list) {
    final pointer = malloc<Float>(list.length);
  ...
  @override
  List<double> utilsWavToNumpyArraywavToNp(dynamic fileData) {
  ...
  void close() {
    ...
    final whisperModelContext = WhisperLibrary._whisperModelContext;
    if (whisperModelContext != null) {
      WhisperLibrary._whisperLibrarySharedBindingsByGeneralDeveloper.whisper_free(whisperModelContext);
    }
  }
